...
export default function GlobeReveal({
  ...
  const lockup = useRef<SVGSVGElement>(null);
  ...
  useGSAP(
    () => {
      ...
      const measure = () => {
        const r = lockup.current!.getBoundingClientRect();
        const sx = r.width / LOCKUP.w;
        const sy = r.height / LOCKUP.h;

        cx = r.left + (GLOBE.x + GLOBE.w / 2 - LOCKUP.x) * sx;
        cy = r.top + (GLOBE.y + GLOBE.h / 2 - LOCKUP.y) * sy;
        rx = (GLOBE.w / 2) * sx;
        ry = (GLOBE.h / 2) * sy;

        // the mark svg is centred on the section, so shift it onto the globe
  ...
  return (
    <section ref={root} className="relative h-svh w-full overflow-hidden bg-[#0E2A47]">
      {/* film */}
      <div ref={film} className="absolute inset-0 will-change-[clip-path]">
        {src ? (
          <video
      ...
      {!done && (
        <svg
          ref={lockup}
          viewBox="200 190 1120 530"
          aria-hidden
          className="pointer-events-none absolute left-1/2 top-1/2"
          style={{ width: LOCKUP_W }}
        >
      ...
      {!done && (
        <svg
          ref={mark}
          viewBox="631 270 265 235"
          aria-hidden
          className="pointer-events-none absolute left-1/2 top-1/2"
          style={{ width: MARK_W, transformOrigin: "50% 50%" }}
        >
          <g transform={TF} fill="#F3EFE9" stroke="none">
